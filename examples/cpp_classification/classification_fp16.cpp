...
class Classifier {
 ...
 private:
  ...
  std::vector<float16> Predict(const cv::Mat& img);
 ...
 private:
  shared_ptr<Net<float16,CAFFE_FP16_MTYPE> > net_;
...
Classifier::Classifier(const string& model_file,
  ...
  /* Load the network. */
  net_.reset(new Net<float16,CAFFE_FP16_MTYPE>(model_file, TEST));
  net_->CopyTrainedLayersFrom(trained_file);

  CHECK_EQ(net_->num_inputs(), 1) << "Network should have exactly one input.";
  CHECK_EQ(net_->num_outputs(), 1) << "Network should have exactly one output.";

  Blob<float16,CAFFE_FP16_MTYPE>* input_layer = net_->input_blobs()[0];
  ...
  Blob<float16,CAFFE_FP16_MTYPE>* output_layer = net_->output_blobs()[0];
  CHECK_EQ(labels_.size(), output_layer->channels())
    << "Number of labels is different from the output layer dimension.";
}

static bool PairCompare(const std::pair<float16, int>& lhs,
                        const std::pair<float16, int>& rhs) {
  return lhs.first > rhs.first;
}

/* Return the indices of the top N values of vector v. */
static std::vector<int> Argmax(const std::vector<float16>& v, int N) {
  std::vector<std::pair<float16, int> > pairs;
...
std::vector<float16> Classifier::Predict(const cv::Mat& img) {
  Blob<float16,CAFFE_FP16_MTYPE>* input_layer = net_->input_blobs()[0];
  ...
  return std::vector<float16>(begin, end);
}
...
void Classifier::WrapInputLayer(std::vector<cv::Mat>* input_channels) {
  ...
  for (int i = 0; i < input_layer->channels(); ++i) {
    cv::Mat channel(height, width, CV_16SC1, input_data);
...
void convertFp16(const cv::Mat& src, cv::Mat& dst) {
  int rows = src.rows;
  int cols = src.cols;
...
void Classifier::Preprocess(const cv::Mat& img,
  ...
  cv::split(sample_fp16, *input_channels);
